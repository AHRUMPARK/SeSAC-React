...
// 그럼 이것도 가능하다.
// initState.explaination[mbtiResult]

// 초기값 설정 마침 > 액션 타입 설정하러 ㄱㄱ
...
// 액션 생성 함수
// 이전에 생각해볼 것은 인자를 받느냐 안받느냐? 결과를 전달해야 하느냐 마느냐에 따라 생성
// 체크는 E,I 를 전달 타입만 전달 할 수 있도록 구현
...
export default function mbti(state = initState, action) {
  switch (action.type) {
    ...
    case NEXT:
      return {
        ...state,
        page: state.page + 1, // 기존값 + 1
      };
    case RESET:
      return {
        ...state,
        mbtiResult: '', // 빈 값 초기화 > 않하면 enfp+i 이런식으로 됨!
        page: 0, // 페이지가 많이 증가되어있을 것을 0 페이지로 돌림
      };
    default:
      return state;
  }
